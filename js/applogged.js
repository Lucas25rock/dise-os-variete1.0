...
document.addEventListener('DOMContentLoaded', function(event){
    if (username){
        ...
        document.getElementById('span-ficha').textContent = username;
        document.getElementById('boton-usuario').addEventListener("click", function(event){
            event.preventDefault();
            //red: logged
            window.location.href = "logged.html?mail=" + username;
        });
        document.getElementById('boton-cierre').addEventListener("click", function(event){
            event.preventDefault();
            //red: logged
